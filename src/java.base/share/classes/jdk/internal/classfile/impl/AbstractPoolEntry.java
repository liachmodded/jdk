    ...
    public static final class Utf8EntryImpl extends AbstractPoolEntry implements Utf8Entry {
        ...
        private void inflate() {
            ...
            try {
                ...
                throw malformedInput(ex);
            }
        }

        private void inflateNonAscii(int singleBytes, int hash) throws UTFDataFormatException {
            ...
            while (px < utfend) {
                int b = rawBytes[px] & 0xff;
                char c;
                if (ModifiedUtf.is1Byte(b)) {
                    c = (char) b;
                    px++;
                } else if (ModifiedUtf.is2Byte(b)) {
                    c = ModifiedUtf.read2Byte(b, rawBytes, px, utfend);
                    px += 2;
                } else if (ModifiedUtf.is3Byte(b)) {
                    c = ModifiedUtf.read3Byte(b, rawBytes, px, utfend);
                    px += 3;
                } else {
                    throw ModifiedUtf.malformedAround(px);
                }
                hash = 31 * hash + c;
        ...
        private ConstantPoolException malformedInput(UTFDataFormatException ex) {
            return new ConstantPoolException("#%d: %s".formatted(index(), ex.getMessage()), ex);
        }
